...
interface BriefViewProps {
  ...
  targetLang: { code: string; name: string; countryCode: string };
}
...
export function BriefView({
  ...
  try {
    ...
    thContent = briefContentTh
      ? (JSON.parse(briefContentTh) as Partial<BriefContent>)
      : tgtContent;
  } catch {
    thContent = {};
    tgtContent = {};
              ...
              {needsTranslation && (
                <div className="flex rounded-xl overflow-hidden border-2 border-accent-brand/30">
                  {(
                    ...
                    <button
                      key={langKey}
                      onClick={() => handleLangSwitch(langKey as 'th' | 'tgt')}
                      disabled={translating}
                      className={`flex items-center gap-1.5 px-3 py-1.5 text-sm font-bold transition-colors disabled:cursor-wait ${
                        viewLang === langKey
                          ? 'bg-accent-brand text-white'
                          : 'bg-transparent text-muted-text'
                      }`}
                    >
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6">
        {translating ? (
          <div className="flex items-center gap-3 px-5 py-4 rounded-2xl bg-white border border-border-ui">
            <Loader2
              size={18}
              className="animate-spin text-accent-brand shrink-0"
            />
            <span className="text-sm font-semibold text-accent-brand">
              กำลังแปล Brief เป็น {targetLang.name}...
            </span>
          </div>
        ) : (
          <>
            <Section
              icon={<FileText size={17} className="text-secondary-brand" />}
              iconBg="bg-secondary-brand-light"
              title="ข้อมูลพื้นฐาน"
            >
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[
                  ...
                  <div key={label}>
                    <p className="text-xs text-muted-text font-semibold uppercase mb-1">
                      {label}
                    </p>
            ...
            <Section
              icon={<Target size={17} className="text-accent-brand" />}
              iconBg="bg-accent-brand-light"
              title="Key Messages"
            >
            ...
            <Section
              icon={<Video size={17} className="text-brand" />}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Section
                ...
                title="Deadline"
              >
                <p className="text-base font-bold text-warning-text">
                  {deadline}
                </p>
              </Section>
              <Section
                icon={
                  <MessageCircle size={17} className="text-secondary-brand" />
                }
                iconBg="bg-secondary-brand-light"
