...
## 5. Dev Mode — Pipeline `accept-test` param

### `src/app/(dashboard)/campaigns/[id]/page.tsx`

You can test the UI states for creator acceptance using the `?accept-test=N` URL param. When in dev mode, passing `?accept-test=3` will force the first 3 creators to show up as "ACCEPTED" traversing the pipeline flow. Works from `1` to `all creators`.

**What to do before production:**
No action needed. The mock is wrapped in a `process.env.NODE_ENV === 'development'` block.
